...
export default function Column1({ info }) {
    ...
    return (<>
        <div className={`${css["profile-col1"]}`}>
            <ImgBlurLoad src={info?.profile_image?.src? SERVER_URL + info.profile_image.src : ""} hash={info?.profile_image?.blur_hash} className={`${css["profile-col1-blur-img"]}`} alt="Фото проживающего"/>
        </div>
    </>);
}